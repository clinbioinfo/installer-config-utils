...
use constant DEFAULT_CONFIG_FILE => "$Find::Bin/../conf/installer_config.ini";
...
my $logger = new InstallerConfig::Logger(
    logfile   => $logfile, 
    log_level => $log_level
);

if (!defined($logger)){
    die "Could not instantiate InstallerConfig::Logger";
}
...
if (!defined($manager)){
    $logger->logdie("Could not instantiate InstallerConfig::Manager");
}
